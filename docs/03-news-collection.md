...
**Google News 한국어 토픽 RSS** (`BUSINESS` + `NATION`) → **당일만** → (조회수 없음) **피드 순서 + 클러스터 + Ollama 중요도**로 상위 기사 선정.
...
- 목표: **실행일(Asia/Seoul) 기사만**
- 토픽 RSS에 `when:1d`를 붙이는 방식은 불안정할 수 있음
- n8n Code/Filter에서 `pubDate` ∈ `[당일 00:00, 실행 시각]` 하드 필터
