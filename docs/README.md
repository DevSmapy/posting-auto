# 문서 목차

이 폴더는 프로젝트의 **상세 설계·운영 문서**입니다. 요약은 루트 [README.md](../README.md)를 보세요.

| 순서 | 문서 | 설명 |
|------|------|------|
| 00 | [MVP 빠른 시작](00-mvp-quickstart.md) | **실행 가이드 (여기부터)** |
| 01 | [개요](01-overview.md) | 무엇을 만드는지, 산출물, 톤, 운영 원칙 |
| 02 | [아키텍처](02-architecture.md) | 파이프라인, 프로세스 구성, 스택 결정 |
| 03 | [뉴스 수집](03-news-collection.md) | Google News MVP, 날짜·중요도, 네이버/검색 RSS 비채택 이유 |
| 04 | [LLM·프롬프트](04-llm-and-prompts.md) | Ollama, 중요도 점수, 브리핑 JSON, 금지 문구 |
| 05 | [발행](05-publishing.md) | 마크다운 반자동, Discord/Telegram Approve, 카드 |
| 06 | [설치·설정](06-setup.md) | 체크리스트, 환경 변수, 로컬 실행 |
| 07 | [워크플로](07-workflow.md) | n8n 노드 순서, DB, 에러 처리 |
| 08 | [로드맵·운영](08-roadmap.md) | 구현 순서, MVP 기준, 트러블슈팅, 보안 |

```text
docs/
├── README.md
├── 00-mvp-quickstart.md
├── 01-overview.md
├── 02-architecture.md
├── 03-news-collection.md
├── 04-llm-and-prompts.md
├── 05-publishing.md
├── 06-setup.md
├── 07-workflow.md
└── 08-roadmap.md
```
