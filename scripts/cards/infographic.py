...
PRETENDARD_PATH = ROOT / "website" / "public" / "fonts" / "PretendardVariable.woff2"
...
def _apply_css_vars(css: str, css_vars: Mapping[str, str] | None) -> str:
    for name, value in (css_vars or {}).items():
        key = name if name.startswith("--") else f"--{name}"
        css = re.sub(rf"{re.escape(key)}:\s*[^;]+;", f"{key}: {value};", css, count=1)
    return css
...
def build_infographic_fields(
    ...
    *,
    ...
    if labels:
        fields.update({key: str(value) for key, value in labels.items() if value})
...
def render_infographic_html(
    ...
    *,
    ...
    catalog: Mapping[str, Pictogram] | None = None,
    labels: Mapping[str, str] | None = None,
    css_vars: Mapping[str, str] | None = None,
) -> tuple[str, dict[str, str], list[PictogramMatch]]:
    fields, matches = build_infographic_fields(
        briefing, brand=brand, date=date, catalog=catalog, labels=labels
    )
    document = TEMPLATE_PATH.read_text(encoding="utf-8")
    document = document.replace(
        "{{DESIGN_SYSTEM_CSS}}",
        _apply_css_vars(CSS_PATH.read_text(encoding="utf-8"), css_vars),
...
def export_infographic(
    ...
    *,
    ...
    document, fields, matches = render_infographic_html(
        briefing, brand=brand, date=date, labels=labels, css_vars=css_vars
    )
    ...
    if render_png:
        ...
        shot = _with_local_font(document) if embed_font else document
        try:
            renderer.screenshot_html(shot, target)
            png_path = target
